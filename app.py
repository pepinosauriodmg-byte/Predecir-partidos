...
# --- FUNCIÓN AUXILIAR: TOP JUGADORES (100% WHITELIST AUTOMÁTICA) ---
def obtener_top_jugadores(equipo, top=3):
    try:
        import re
        df_jugadores = pd.read_csv('rendimiento_jugadores.csv')
        ...
        # ==========================================
        # 🚧 FILTRO ÚNICO: WHITELIST ESTRICTA FIFA 🚧
        # ==========================================
        try:
            df_oficial = pd.read_csv('convocados_oficiales.csv')
            
            # Normalizador: destruye mayúsculas y acentos para un match perfecto
            ...
            if not plantilla_oficial.empty:
                nombres_of_limpios = [normalizar_texto(n) for n in plantilla_oficial['Jugador_Oficial'].tolist()]
                
                def es_convocado(nombre_db):
                    nom_db_limpio = normalizar_texto(nombre_db)
                    for nom_oficial in nombres_of_limpios:
                        # Regex estricto: \b garantiza que "Corona" no se confunda con "Coronado"
                        if re.search(rf'\b{nom_oficial}\b', nom_db_limpio):
                            return True
                    return False
                
                # LA REGLA DE ORO: Filtro destructivo sin vuelta atrás.
                ...
                if not plantilla_filtrada.empty:
                    ...
                    return [f"⚠️ Bloqueo: No logramos empatar los nombres de {equipo} con el PDF de la FIFA."]
                    
        except FileNotFoundError:
            pass # Si el CSV oficial no existe, la app debe seguir funcionando (con la base sucia)
        # ==========================================
        
        # Lógica matemática de rating
        def calcular_rating_real(fila):
            pos = str(fila[col_pos]).upper()
            atq = pd.to_numeric(fila[col_rat_atq], errors='coerce')
            dfn = pd.to_numeric(fila[col_rat_def], errors='coerce')
        ...
        return [f"Error del sistema: {str(e)[:40]}"]
